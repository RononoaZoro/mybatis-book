...
public abstract class AbstractEnhancedDeserializationProxy {
  ...
  private final Map<String, ArcherResultLoaderMap.LoadPair> unloadedProperties;
  ...
  protected AbstractEnhancedDeserializationProxy(Class<?> type, Map<String, ArcherResultLoaderMap.LoadPair> unloadedProperties,
        ...
        synchronized (this.reloadingPropertyLock) {
          if (!FINALIZE_METHOD.equals(methodName) && PropertyNamer.isProperty(methodName) && !reloadingProperty) {
            ...
            if (unloadedProperties.containsKey(propertyKey)) {
              final ArcherResultLoaderMap.LoadPair loadPair = unloadedProperties.remove(propertyKey);
              if (loadPair != null) {
                try {
  ...
  protected abstract AbstractSerialStateHolder newSerialStateHolder(
          Object userBean,
          Map<String, ArcherResultLoaderMap.LoadPair> unloadedProperties,
