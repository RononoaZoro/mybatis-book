...
import com.luo.ibatis.scripting.defaults.ArcherRawSqlSource;
 ...
 */
public class ArcherXMLScriptBuilder extends ArcherBaseBuilder {
  ...
  public ArcherXMLScriptBuilder(ArcherConfiguration configuration, XNode context) {
    this(configuration, context, null);
  }

  public ArcherXMLScriptBuilder(ArcherConfiguration configuration, XNode context, Class<?> parameterType) {
    super(configuration);
  ...
  public ArcherSqlSource parseScriptNode() {
    // 调用parseDynamicTags（）方法將SQL配置转换为SqlNode对象
    ArcherMixedSqlNode rootSqlNode = parseDynamicTags(context);
    ArcherSqlSource sqlSource = null;
    // 判断Mapper SQL配置中是否包含动态SQL元素，如果是创建DynamicSqlSource对象，否则创建RawSqlSource对象
    if (isDynamic) {
      sqlSource = new DynamicSqlSource(configuration, rootSqlNode);
    } else {
      sqlSource = new ArcherRawSqlSource(configuration, rootSqlNode, parameterType);
    }
    return sqlSource;
  }

  protected ArcherMixedSqlNode parseDynamicTags(XNode node) {
...
//     对XML子元素进行遍历
    ...
    return new ArcherMixedSqlNode(contents);
  }
  ...
  private class TrimHandler implements NodeHandler {
    ...
    @Override
    public void handleNode(XNode nodeToHandle, List<SqlNode> targetContents) {
      ArcherMixedSqlNode mixedSqlNode = parseDynamicTags(nodeToHandle);
