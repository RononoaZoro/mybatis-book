...
public class JndiDataSourceFactoryTest extends BaseDataTest {
  ...
  private ArcherUnpooledDataSource expectedDataSource;
  ...
  public void shouldRetrieveDataSourceFromJNDI() throws Exception {
    createJndiDataSource();
      ArcherJndiDataSourceFactory factory = new ArcherJndiDataSourceFactory();
    factory.setProperties(new Properties() {
      {
        setProperty(ArcherJndiDataSourceFactory.ENV_PREFIX + Context.INITIAL_CONTEXT_FACTORY, TEST_INITIAL_CONTEXT_FACTORY);
        setProperty(ArcherJndiDataSourceFactory.INITIAL_CONTEXT, TEST_INITIAL_CONTEXT);
        setProperty(ArcherJndiDataSourceFactory.DATA_SOURCE, TEST_DATA_SOURCE);
      }
    });
